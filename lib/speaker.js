...
Speaker = (function(_super) {
  ...
  function Speaker(addresses) {
    ...
    this.addresses = addresses.length;
    this.sendAfterConnect = [];
  ...
  Speaker.prototype.connect = function(address) {
    ...
    socket.connect(port, host, function() {
      ...
      if (_this.sendAfterConnect.length) {
        _this.sendAfterConnect.forEach(function(args) {
          Speaker.send.apply(_this, args);
        });
        this.sendAfterConnect = [];
      }
    });
  ...
  Speaker.prototype.send = function(subject, data, callback) {
    ...
    if (this.sockets.length === 0) {
      if (this.addresses) {
        this.sendAfterConnect.push([subject, data, callback]);
        return;
      }
      if (callback) {
        callback({
