...
Speaker = (function(_super) {
  ...
  Speaker.prototype.connect = function(address) {
    ...
    socket.on('data', function(data) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        delete _this.waiters[message.id];
        _results.push(delete _this.waitersToFlush[message.id]);
      }
      return _results;
    });
  ...
  Speaker.prototype.request = function(subject, data, callback) {
    ...
    this.waitersToFlush[messageId] = true;
  ...
  Speaker.prototype.generateUniqueId = function() {
    var id;
    id = 'id-' + this.uniqueId;
    if (!this.waiters[id]) return id;
    if (this.uniqueId++ === MAX_WAITERS) this.uniqueId = 1;
    if (this.waitersToFlush[this.uniqueId]) {
      delete this.waitersToFlush[this.uniqueId];
      delete this.waiters[this.uniqueId];
    }
